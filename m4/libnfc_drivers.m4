...
[
  ...
  case "${DRIVER_BUILD_LIST}" in
    default)
                  DRIVER_BUILD_LIST="acr122_usb acr122s arygon pn53x_usb pn532_uart pn532_net"
                  ...
                  ;;
    all)
                  DRIVER_BUILD_LIST="acr122_pcsc acr122_usb acr122s arygon pn53x_usb pn532_uart pn532_net"
                  if test x"$spi_available" = x"yes"
                  then
                      DRIVER_BUILD_LIST="$DRIVER_BUILD_LIST pn532_spi"
                  fi
  ...
  do
    ...
    pn532_net)
                  net_required="yes"
                  driver_pn532_net_enabled="yes"
                  DRIVERS_CFLAGS="$DRIVERS_CFLAGS -DDRIVER_PN532_NET_ENABLED"
                  ;;
    pn532_spi)
                  spi_required="yes"
  ...
  AM_CONDITIONAL(DRIVER_PN532_NET_ENABLED, [test x"$driver_pn532_net_enabled" = xyes])
...
echo "   pn532_net........ $driver_pn532_net_enabled"
